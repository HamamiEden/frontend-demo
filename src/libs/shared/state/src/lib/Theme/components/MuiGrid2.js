const colorOptions = {
  light: {
    ...
    recommendationGradient: {
      background: "linear-gradient(275.53deg, rgba(214, 241, 254, 0.4) 0%, rgba(255, 255, 255, 0.4) 98.71%)",
      before: {
        background: "linear-gradient(90deg, #EDEEF0 0%, #D6F1FE 100%)"
      }
    }
  },
  dark: {
    ...
    recommendationGradient: {
      background: "linear-gradient(275.53deg, rgba(15, 61, 82, 0.4) 0%, rgba(26, 28, 30, 0.4) 98.71%)",
      before: {
        background: "linear-gradient(90deg, #5B626E 0%, #0F3D52 100%)"
      }
    }
  }
};
...
const MuiGrid2 = mode => {
  return {
    variants: [
      ...
      {
        props: { border_color: "spaceGray" },
        style: {
          border: `1px solid ${colorOptions[mode].spaceGray}`
        }
      },
      {
        props: { variant: "recommendationItem" },
        style: {
          position: "relative",
          background: colorOptions[mode].recommendationGradient.background,
          borderRadius: "8px",
          "&::before": {
            content: '""',
            position: "absolute",
            inset: 0,
            padding: "1px",
            background: colorOptions[mode].recommendationGradient.before.background,
            borderRadius: "8px",
